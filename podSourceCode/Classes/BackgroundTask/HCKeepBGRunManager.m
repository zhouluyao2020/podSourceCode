...
- (instancetype)init {
    if (self = [super init]) {
        ...
        //静音文件
        NSBundle *libBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[self class]] pathForResource:@"ZGDownloader" ofType:@"bundle"]];
        NSString *filePath = [libBundle pathForResource:@"JM" ofType:@"wav"];
...
- (void)startBGRun{
    ...
    dispatch_async(_queue, ^{
        self.timerLog = [[NSTimer alloc] initWithFireDate:[NSDate date] interval:10 target:self selector:@selector(log) userInfo:nil repeats:YES];
